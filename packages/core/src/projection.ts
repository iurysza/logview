...
import { clipToWidth, type ClippedText } from "./display-text.ts";
...
type ColumnLayout = Readonly<{
	showPid: boolean;
	tagWidth: number;
	messageWidth: number;
}>;

function layoutColumns(columns: number): ColumnLayout {
...
function clipField(text: string, width: number): ClippedText {
