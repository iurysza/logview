...
- Run `logview replay PATH --speed instant --headless`.
- Run `logview live --headless --serial DEVICE` with `--adb` pointed at a stub.
- Run `logview live --headless` with the no-device or multi-device stub.

## Driving it with logview
...
- **Replay summary.** Run `bun run logview replay tests/fixtures/real/sanitized-aosp-pattern.lvr.jsonl --speed instant --headless`. Exit code `0`. The last stdout line is JSON with `version` `1`, `kind` `"summary"`, `terminal.kind` `"ended"`, and `snapshot.stats.admittedEvents` `15`.
- **Live stub.** Run `bun run logview live --headless --adb tests/support/adb-stubs/one-device --serial emulator-5554`. Exit code `0`. The summary `terminal.kind` is `"ended"` and `admittedEvents` is `15`.
- **No device.** Run `bun run logview live --headless --adb tests/support/adb-stubs/no-device`. Exit code `1`. Stderr or stdout names the missing-device failure.
- **Several devices.** Run `bun run logview live --headless --adb tests/support/adb-stubs/multi-device`. Exit code `1`. The output contains `ambiguous-device`.
...
- Recording is a separate command (`logview record --out PATH`). It is not this feature. Do not write recordings into `tests/fixtures/`.
