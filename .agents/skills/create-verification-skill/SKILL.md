...
description: Generates a project-local verification skill that drives an app the way a user does across any language, framework, or platform. Use only when the user explicitly asks to create a verification or control skill for a repository that lacks a scripted way to prove UI, CLI, or service behavior.
metadata:
